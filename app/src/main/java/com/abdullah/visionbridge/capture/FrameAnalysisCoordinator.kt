...
class FrameAnalysisCoordinator(
    ...
    private suspend fun processStableText(
        ...
        // With Gemini available, local Latin OCR is same-frame evidence and a visual preview. It is
        // not spoken first because that would destroy the mixed Arabic/English visual order.
    ...
    /**
     * Local OCR finishes immediately. Gemini streams from a copied snapshot in a latest-only queue,
     * so cloud latency cannot block detection of fast captions, tickers, or scrolling text.
     */
    ...
    /**
     * Scene description is an independent live pipeline. It never waits for the text stability gate
     * and never blocks capture while Gemini is speaking. While one stream is active, only the newest
     * changed scene is retained, eliminating both silence and stale backlogs.
     */
