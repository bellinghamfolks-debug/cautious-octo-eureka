...
/**
 * Plays Gemini Live native audio with a deliberately tiny buffer.
 *
 * Accessibility live-view audio must prefer freshness over completeness. When a new visual turn
 * starts, the epoch advances and AudioTrack is flushed so stale speech cannot continue over the new
 * scene. Gemini Live native audio is mono, 16-bit little-endian PCM at 24 kHz.
 */
class LivePcmAudioPlayer {
    ...
    private val audioTrack: AudioTrack = createTrack()
    ...
    /** Starts a fresh spoken visual turn and drops any buffered speech from the previous turn. */
    fun beginTurn(): Long {
        val newEpoch = epoch.incrementAndGet()
        flushTrack("new_live_turn")
        return newEpoch
    }

    fun enqueue(packetEpoch: Long, bytes: ByteArray) {
        if (bytes.isEmpty() || packetEpoch != epoch.get()) return
        val result = packets.trySend(Packet(packetEpoch, bytes))
        if (result.isFailure) {
    ...
    fun interrupt(reason: String = "visual_target_changed") {
    ...
    private fun flushTrack(reason: String) {
        synchronized(trackLock) {
    ...
    private fun createTrack(): AudioTrack {
        ...
        val bufferBytes = max(minBuffer * 2, FALLBACK_BUFFER_BYTES)
